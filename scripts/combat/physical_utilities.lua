...
xi.combat.physical.calculateTPScaling = function(actorTP, tpModifierTable)
    if not tpModifierTable or #tpModifierTable == 0 then
        return 0
    end
    ...
    for i = 1, #tpModifierTable - 1 do
        ...
        if actorTP <= upperBreakpoint.tp then
            -- If a coefficient is supplied, use it as the modifier
            -- increase per 100 TP for this segment.
            if lowerBreakpoint.coefficient then
                return lowerBreakpoint.modifier +
                    ((actorTP - lowerBreakpoint.tp) / 100) *
                    lowerBreakpoint.coefficient
            end

            -- Otherwise use standard linear interpolation between
            -- the lower and upper breakpoint modifiers.
            return lowerBreakpoint.modifier +
