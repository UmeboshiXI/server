...
enum MOBMODIFIER : int
{
    ...
    MOBMOD_BASE_DAMAGE_OVERRIDE    = 86, // Overrides the mob's base damage.
    MOBMOD_BASE_DAMAGE_MULTIPLIER  = 87, // Multiplies the mob's base damage. This is applied BEFORE MOBMOD_BASE_DAMAGE_OFFSET. BASE 10000 (15000 = x1.5)
    MOBMOD_DAMAGE_MULTIPLIER_MODE  = 88, // Default: 0. Setting to 1 includes the damage offset in the MOBMOD_BASE_DAMAGE_MULTIPLIER. Used for special mob cases such as Korrigan and Tauri.
    MOBMOD_DAMAGE_OFFSET           = 89, // Overrides the mob's base damage offset.
    MOBMOD_RANGED_DAMAGE_OFFSET    = 90, // Overrides the mob's ranged base damage offset.
    MOBMOD_FAMILY_DAMAGE_OFFSET    = 91, // Adjusts the existing offset after region/mod corrections (Some mob families have slight differences in offset).
};
