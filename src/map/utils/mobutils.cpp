...
namespace mobutils
{
    ...
    uint16 GetWeaponDamage(CMobEntity* PMob, uint16 slot)
    {
        ...
        // Override the offset if mobmod value is defined.
        if (PMob->getMobMod(MOBMOD_DAMAGE_OFFSET) != 0)
        {
        ...
        if (slot == SLOT_RANGED)
        {
            ...
            // Override the offset if mobmod value is defined.
            if (PMob->getMobMod(MOBMOD_RANGED_DAMAGE_OFFSET) != 0)
            {
        ...
        if (mobZoneId != 0 && PMob->m_Type != MOBTYPE_NOTORIOUS && (mobZoneId == ZONE_WEST_RONFAURE ||
                               mobZoneId == ZONE_EAST_RONFAURE ||
                               mobZoneId == ZONE_NORTH_GUSTABERG ||
                               mobZoneId == ZONE_SOUTH_GUSTABERG ||
                               mobZoneId == ZONE_WEST_SARUTABARUTA ||
                               mobZoneId == ZONE_EAST_SARUTABARUTA))
        {
            offset -= 1;
        }

        // Add/Subtract mob family offsets.
        if (PMob->getMobMod(MOBMOD_FAMILY_DAMAGE_OFFSET) != 0)
        {
            offset += PMob->getMobMod(MOBMOD_FAMILY_DAMAGE_OFFSET);
        }
        ...
        // Override the mob's base damage if mobmod value is defined.
        if (PMob->getMobMod(MOBMOD_BASE_DAMAGE_OVERRIDE) != 0)
        {
            damage = PMob->getMobMod(MOBMOD_BASE_DAMAGE_OVERRIDE);
        }

        // Add this mod to increase a mobs damage by a base amount
        if (PMob->getMobMod(MOBMOD_WEAPON_BONUS) != 0)
        {
            damage = (uint16)(damage + PMob->getMobMod(MOBMOD_WEAPON_BONUS));
        }

        // Add damage mods to the appropriate slot's base damage if the mob has them.
        if (slot == SLOT_MAIN)
        {
            damage += PMob->getMod(Mod::MAIN_DMG_RATING);
        }
        else if (slot == SLOT_SUB)
        {
            damage += PMob->getMod(Mod::SUB_DMG_RATING);
        }
        else if (slot == SLOT_RANGED)
        {
            damage += PMob->getMod(Mod::RANGED_DMG_RATING);
        }

        // Override the base damage multiplier if mobmod value is defined.
        if (PMob->getMobMod(MOBMOD_BASE_DAMAGE_MULTIPLIER) != 0)
        {
            multiplier = PMob->getMobMod(MOBMOD_BASE_DAMAGE_MULTIPLIER) / 10000.0f;
        }

        // MOBMOD_DAMAGE_MULTIPLIER_MODE determines which damage formula we use for multiplying.
        if (PMob->getMobMod(MOBMOD_DAMAGE_MULTIPLIER_MODE) == 1)
        {
            // Special cases(Korrigan/Tauri mobs) add the offset BEFORE multiplying.  
            damage = (uint16)((damage + offset) * multiplier);
        }
        else
        {
            // Normal mobs typically add the offset AFTER multiplying.
            damage = (uint16)(damage * multiplier + offset);
        }
